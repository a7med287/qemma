...
class _AssistantDashboardContentState extends State<_AssistantDashboardContent> {
  String _searchQuery = '';
  List<AssistantStudent> _students = [];
  ...
  Future<void> _loadStudents() async {
    ...
    try {
      ...
      if (mounted) setState(() { _students = students; _studentsLoading = false; });
    } catch (e) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 12.h),
            Text(_searchQuery.isNotEmpty ? 'لا توجد نتائج بحث' : 'لا يوجد طلاب مسجلين بعد',
                style: TextStyle(fontFamily: 'Cairo', fontSize: 14.sp, color: isDark ? const Color(0xFF94A3B8) : const Color(0xFF6B7280))),
          ],
        ),
      );
    }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (student.enrolledCourses.isNotEmpty)
                          Text('${student.enrolledCourses.length} كورسات مسجل بها',
                              style: TextStyle(fontSize: 11.sp, fontFamily: 'Cairo',
                                  color: isDark ? const Color(0xFF94A3B8) : const Color(0xFF6B7280))),
                        if (student.division != null)
                          Container(
                            ...
                            child: Text(student.division!,
                                style: TextStyle(fontSize: 10.sp, fontWeight: FontWeight.w600, fontFamily: 'Cairo', color: const Color(0xFF2563EB))),
                          ),
                      ],
                    ),
                  ),
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        icon: Icon(Icons.close, color: isDark ? const Color(0xFF94A3B8) : const Color(0xFF6B7280)),
                      ),
                    ],
                  ),
                ),
                Flexible(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (student.email != null) _detailRow('البريد', student.email!, isDark),
                        if (student.phone != null) _detailRow('الهاتف', student.phone!, isDark),
                        if (student.division != null) _detailRow('القسم', student.division!, isDark),
                        if (student.username != null) _detailRow('اسم المستخدم', student.username!, isDark),
                        if (student.enrolledCourses.isNotEmpty) ...[
                          SizedBox(height: 12.h),
                          Text('الكورسات المسجل بها:',
                              style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.bold, fontSize: 13.sp,
                                  color: isDark ? const Color(0xFFF1F5F9) : const Color(0xFF1E293B))),
                          SizedBox(height: 8.h),
                          ...student.enrolledCourses.map((c) => Padding(
