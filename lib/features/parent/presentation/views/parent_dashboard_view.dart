...
class _ParentDashboardViewState extends State<ParentDashboardView> {
  ...
  // ---------------------------------------------------------------------
  // FIX: the dialog used to be built inline with a StatefulBuilder, and
  // the TextEditingControllers were disposed immediately after
  // showDialog()'s Future resolved (right after Navigator.pop). That is
  // too early: the dialog's exit transition is still animating/rebuilding
  // for a few more frames after the Future completes, and those frames
  // still reference the controllers -> "used after being disposed" crash,
  // which then cascades into the Directionality `_dependents.isEmpty`
  // assertion.
  //
  // The fix is to make the dialog its own StatefulWidget (_AddChildDialog
  // below) that owns its controllers and disposes them in its own
  // dispose(). Flutter only calls State.dispose() once the element is
  // truly unmounted (after the exit animation finishes), so there's no
  // more manual timing to get right.
  // ---------------------------------------------------------------------
...
/// Self-contained "add child" dialog. Owns `usernameCtrl` / `codeCtrl` and
/// disposes them in `dispose()`, so cleanup always happens at the correct
/// point in the widget lifecycle (after the dialog has actually been
/// unmounted), instead of being raced against the closing animation.
