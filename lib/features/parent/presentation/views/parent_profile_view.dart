  ...
  Widget _buildAddPassword() {
    ...
    return StatefulBuilder(
      builder: (context, setLocal) => Column(
        children: [
          Text(
            'ليس لديك كلمة مرور. أضف واحدة لحماية حسابك.',
          ...
          ElevatedButton(
            onPressed: saving ? null : () async {
              if (pwdCtrl.text.length < 8) {
                buildSnackBar(context, 'كلمة المرور يجب أن تكون 8 أحرف على الأقل', isError: true);
                return;
              }
              if (pwdCtrl.text != confirmCtrl.text) {
                buildSnackBar(context, 'كلمتا المرور غير متطابقتين', isError: true);
                return;
              }
              setLocal(() => saving = true);
              try {
                await context.read<AuthService>().addPassword(pwdCtrl.text);
                if (mounted) buildSnackBar(context, 'تم إضافة كلمة المرور بنجاح');
              } catch (e) {
                if (mounted) buildSnackBar(context, 'فشل إضافة كلمة المرور', isError: true);
              } finally {
            ...
            child: saving
                ...
                : const Text('إضافة كلمة المرور', style: TextStyle(fontFamily: 'Cairo')),
          ),
        ],
      ),
    );
