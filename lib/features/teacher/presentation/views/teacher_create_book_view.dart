...
class TeacherCreateBookView extends StatefulWidget {
  static const routeName = '/teacher/books/create';
  const TeacherCreateBookView({super.key});

  @override
  State<TeacherCreateBookView> createState() => _TeacherCreateBookViewState();
}

class _TeacherCreateBookViewState extends State<TeacherCreateBookView> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await context.read<TeacherRepository>().createBook(
      ...
      _showToast('فشل إضافة الكتاب', error: true);
      setState(() => _submitting = false);
    }
  }
  ...
  Future<void> _pickCover() async {
    ...
    if (file.bytes != null && file.bytes!.length > 2 * 1024 * 1024) {
      _showToast('حجم الصورة يجب أن يكون أقل من 2MB', error: true);
      return;
    }
    final ext = file.extension ?? 'png';
    setState(() => _coverBase64 = 'data:image/$ext;base64,${base64Encode(file.bytes!)}');
  }
    ...
    return Row(
      children: [
        ...
        Column(
          ...
          children: [
            Text('إضافة كتاب جديد', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w900, fontSize: 18.sp, color: isDark ? const Color(0xFFF1F5F9) : const Color(0xFF0F172A))),
            Text('أضف كتاباً دراسياً جديداً لطلابك', style: TextStyle(fontFamily: 'Cairo', fontSize: 11.sp, color: isDark ? const Color(0xFF94A3B8) : const Color(0xFF6B7280))),
          ],
        ),
      ],
    );
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          value: _grade.isNotEmpty ? _grade : null,
        ...
        OutlinedButton.icon(
          ...
          label: Text(_coverBase64 != null ? 'تغيير صورة الغلاف' : 'رفع صورة الغلاف (اختياري)', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w700, fontSize: 11.sp)),
        ...
        if (_coverBase64 != null) ...[
          ...
          ClipRRect(
            ...
            child: Stack(
              children: [
                Image.memory(base64Decode(_coverBase64!.split(',').last), fit: BoxFit.cover, width: double.infinity, height: 120.h),
                Positioned(
                  top: 4, right: 4,
                  child: GestureDetector(
                    onTap: () => setState(() => _coverBase64 = null),
                    child: Container(
          ...
          child: ElevatedButton(
            ...
            child: _submitting
                ? Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    SizedBox(width: 8.w),
                    const Text('جاري الإضافة...', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w900, fontSize: 14, color: Colors.white)),
                  ])
                : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    const Icon(Icons.add, size: 18, color: Colors.white),
                    SizedBox(width: 8.w),
                    const Text('إضافة الكتاب', style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.w900, fontSize: 14, color: Colors.white)),
                  ]),
          ),
        ),
      ],
